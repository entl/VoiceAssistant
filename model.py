#created by NeuralNine copied code from pypi 
#because don't want to edit exactly module
#but corrections was required
...
class IAssistant(metaclass=ABCMeta):
    ...
    @abstractmethod
    def request_function_with_command(self, message, command):
        """ Implemented in child class """

    @abstractmethod
    def request_function_with_response(self, message):
        """ Implemented in child class """

    @abstractmethod
    def request_function_without_params(self, message):
        """ Implemented in child class """
...
class GenericAssistant(IAssistant):
    ...
    def get_intent_method_name(self, message):
        ints = self._predict_class(message)
        return self.intent_methods[ints[0]['intent']].__name__, ints[0]['intent']

    def request_function_with_command(self, message, command=None):
        ints = self._predict_class(message)

        # try:
        if ints[0]['intent'] in self.intent_methods.keys():
            return self._get_response(ints, self.intents), self.intent_methods[ints[0]['intent']](command)
        else:
                return self._get_response(ints, self.intents)
        # except:
        #     if ints[0]['intent'] in self.intent_methods.keys():
        #         return self._get_response(ints, self.intents), self.intent_methods[ints[0]['intent']]()
        #     else:
        #         return self._get_response(ints, self.intents)
    ...
    def request_function_with_response(self, message):
        ...
        if ints[0]['intent'] in self.intent_methods.keys():
            ...
            return response, self.intent_methods[ints[0]['intent']](response)
        else:
            return self._get_response(ints, self.intents)
